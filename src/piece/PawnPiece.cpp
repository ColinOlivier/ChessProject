...
std::vector<BoardTile*> PawnPiece::getPossibleMove() const
{
    ...
    if (oneStepAhead.getPiece() == nullptr) // vérifie si la case devant est vide
    {
        ...
        if ((getPlayerColor() == PlayerColor::White && y == 1) || (getPlayerColor() == PlayerColor::Black && y == 6))
        {
            ...
            if (twoStepsAhead.getPiece() == nullptr)
    ...
    if (leftDiagonal.getPiece() && leftDiagonal.getPiece()->getPlayerColor() != getPlayerColor()) // vérifie que le pion à catpurer n'est pas de la meme couleur, que la case est valide et qu'elle contient une pièce
    {
        possibleMoves.push_back(&leftDiagonal);
    }

    if (rightDiagonal.getPiece() && rightDiagonal.getPiece()->getPlayerColor() != getPlayerColor())
    {
